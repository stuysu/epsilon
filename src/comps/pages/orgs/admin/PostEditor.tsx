...
const PostEditor = (
    { content, orgId, onSave, onCancel, onCreate } : 
    { content? : Post, orgId: number, onSave? : (newData : Post) => void, onCancel? : () => void, onCreate? : (newData : Post) => void }
) => {
    ...
    const createPost = async () => {
        ...
        if (error || !data) {
            return user.setMessage("Could not create post. Please contact it@stuysu.org for support.");
        }
    ...
    const savePost = async () => {
        ...
        if (error || !data) {
            return user.setMessage("Could not update post. Please contact it@stuysu.org for support.");
        }
            ...
            <Box bgcolor="background.default" color="primary.contrastText">
                {editableFieilds.map((field, i) => {
                    ...
                    return (
                        <Box key={i}>
                            {field}
                            <TextField value={postData[key] || ""} name={field} onChange={onChange} />
                        </Box>
                    )
                })}
            </Box>
