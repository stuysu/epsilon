...
const AdminUpsertMeeting = (
    ...
    const [startTime, setStartTime] = useState<Dayjs | null>(start ? dayjs(start) : null)
    ...
    useEffect(() => {
        const fetchRooms = async () => {
            ...
            if (error) {
                user.setMessage("Error fetching rooms. Contact it@stuysu.org for support.")
                return;
            }
    ...
    const handleSave = async () => {
        ...
        if (supabaseReturn.error) {
            user.setMessage("Error creating meeting. Contact it@stuysu.org for support.");
            return;
        }
    ...
    return (
        ...
        >
            ...
            <DialogContent>
                <TextField 
                    value={meetingTitle} 
                    onChange={(event: ChangeEvent<HTMLInputElement>) => setMeetingTitle(event.target.value)} 
                    label="Title"
                />
                <TextField 
                    value={meetingDesc} 
                    onChange={(event: ChangeEvent<HTMLInputElement>) => setMeetingDesc(event.target.value)} 
                    label="Description"
                />
                ...
                >
                    ...
                    {
                        availableRooms.map(room => (<MenuItem key={room.id} value={String(room.id)}>{room.name}</MenuItem>))
                    }
                </Select>
                ...
                <FormControlLabel
                    control={
                        <Switch
                            checked={isPub}
                            onChange={(event: ChangeEvent<HTMLInputElement>) => setIsPub(event.target.checked)}
                        />
                    }
                    label="is public?"
                />
