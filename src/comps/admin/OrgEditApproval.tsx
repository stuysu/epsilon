...
const editFields : EditKey[] = ["name", "url", "picture", "mission", "purpose", "benefit", "appointment_procedures", "uniqueness", "meeting_schedule", "meeting_days", "commitment_level", "keywords"]

const OrgEditApproval = (
    { onBack, onApprove, ...edit }: 
    ...
    useEffect(() => {
        ...
        const fetchCurrentOrg = async () => {
            ...
            if (error || !data) {
                return user.setMessage("Could not fetch current organization data. Please contact it@stuysu.org for support.")
            }
    ...
    const approve = async () => {
        ...
        if (error) {
            return user.setMessage("Error updating organization. Contact it@stuysu.org for support.")
        }
        ...
        if (error) {
            return user.setMessage("Error deleting old organization edit. Please let it@stuysu.org know about this issue.")
        }
