...
const Credit = ({
    ...
    return (
        <div className={"max-w-3xl w-full sm:mt-16 mt-0"}>
            <div className={"relative flex flex-col items-center pb-6 sm:pb-12"}>
                <div
...
const MemberGroup = ({ role, names }: { role: string; names: string[] }) => {
    return (
        <div className={"flex sm:flex-row gap-2 sm:gap-5 items-start mb-3 flex-col"}>
            <p
                style={{
                    fontFamily: `'neue-haas-grotesk-display', sans-serif`,
                }}
            ...
            >
                {names.map((name, index) => (
                    <h2 className={"-mt-3 text-white/75 relative top-0.5"} key={index}>
                        {name}
                    </h2>
                ))}
            </div>
